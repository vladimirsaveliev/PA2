...
CSearch<_Type, _Comparator>::CSearch(void) : m_Needles(), m_Comp()
{
}

template <typename _Type, typename _Comparator>
CSearch<_Type, _Comparator>::CSearch(_Comparator c) : m_Needles(), m_Comp(c)
{
}
...
{
    m_Needles.push_back(make_pair(id, needle));
}
...
set<int> CSearch<_Type, _Comparator>::Search(const _Type & hayHeap) const
{
    ...
    for (auto x: m_Needles) {
        if (search(hayHeap.begin(), hayHeap.end(),
                   x.second.begin(), x.second.end(), m_Comp) != hayHeap.end())
            rc.insert(x.first);
    }
    return rc;
}
